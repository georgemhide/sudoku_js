...
// jQuery bound function

jQuery.fn.sudoku = function() {
	const table = new sTable();
	table.getCssLinkElement().appendTo("head");
	table.generateTable(this);

	this.initSudoku(table);
	table.generateTableCells();

	$("body table").on("focus","input.userIn",function() {
		$(this).putCursorAtEnd();
		table.highlightCells($(this));
	});

	$("body table").on("input","input.userIn", function() {
		let newValue = $(this).val();
		const _cell = table.getCell(this);

		if (_cell.randomized) {
			$(this).val(_cell.inputVal);
			return;
		}

		if(isNaN(newValue) || newValue.length > 1) {
			alert("Only numeric values from 1-9 are allowed!");
			$(this).val(undefined);
...
jQuery.fn.initSudoku = function(table) {

	let firstNumbers = 51;
	const cellCoordinates = [];
	for (var i = 0; i < firstNumbers; i++) {
		const randomX = Math.floor(Math.random() * 9);
		const randomY = Math.floor(Math.random() * 9);
		coords = `${randomY}${randomX}`;
		
		if (cellCoordinates.includes(coords)) {
			continue;
		}

		cellCoordinates.push(coords);
		i++;
	}

	cellCoordinates.forEach(coords => table.initializeRandomCell(coords));
...
class sTable {
	...
	#createCell(inputCoords) {
		return "<td>"+this.#createInput(inputCoords)+"</td>";
	}

	#createInput(inputCoords) {
		return `<input id=${inputCoords} type='text' class='userIn'>`;
	}
	...
	#getCoordsFromElement(element) {
		const col = $(element).parent().index();
		const row = $(element).parent().parent().index();

		return {row, col};
	}
	
	#initCell(element, y, x) {
		const coords = `${y}${x}`;
		const _cell = new cell(parseInt($(element).val()), coords, false);
	...
	generateTable(table) {
		for (let y = 0; y < 9; y++) {
			...
			for (let x = 0; x < 9; x++)
				$row.append(this.#createCell(`${y}${x}`));
		}
	}

	getCell(element) {
		const {row, col} = this.#getCoordsFromElement(element);

		return this.inputs[`${row}${col}`] ?? this.#initCell(element, row, col);
	}

	generateTableCells() {
		Object.values(this.inputs).forEach(cell => {
			const $table = $("table");
			const $input = $table.find(`input#${cell.y}${cell.x}`).first();
			
			$input.val(cell.inputVal);
			$input.addClass("default");
		});
	}

	initializeRandomCell(coords) {
		const randomValue = Math.floor(Math.random() * 9) + 1;

		let _cell = new cell(randomValue, coords, true);

		if (_cell.hasError(this.inputs)) {
			_cell = undefined;
			return;
		}
	...
	highlightCells(element) {
		...
		const x = element.parent().index();
		const y = element.parent().parent().index();

		$siblings.each(function() {
			$("input", this).addClass("highlight");
		});

		$("table tr").each(function() {
			if ($(this).index() == y) return true;

			$(this).find("td input").eq(x).addClass("highlight");
		});
	}

	setBorder() {
		$("table tr:first-child td").each(function() {
			$(this).css("border-top","2px dotted black");
		});

		$("table tr:nth-child(3n+3) td").each(function() {
			$(this).css("border-bottom","2px dotted black");
		});

		$("table tr td").each(function() {
			if (($(this).index() + 1) % 3 == 0 && $(this).index() != 0) {
				$(this).css("border-right", "2px dotted black");
			}
		});

		$("table tr td:first-child").each(function() {
			$(this).css("border-left", "2px dotted black");
		});
	}

	pritnCurrentTableAsArray() {
		let coords;
		let result = "[\n";
		for (let row = 0; row < 9; row++) {
			...
			for (let column = 0; column < 9; column++) {
				...
				coords = `${row}${column}`;
				result += `${this.inputs[coords]?.inputVal ?? 0},`;
			}
			result += "],\n";
		}
...
class cell {
	...
	constructor(inputVal, coords, randomized) {
		this.y = parseInt(coords.charAt(0));
		this.x = parseInt(coords.charAt(1));
	...
	#getValues(inputs, index, row) {
		const values = [];

		for (var i = 0; i < 9; i++) {
			if (index === i) continue;

			const coords = (row) ? `${this.y}${i}` : `${i}${this.x}`;

			if (!!inputs[coords]) {
				values.push(inputs[coords].inputVal);
			}
		}
	...
	#getBoxValues(inputs) {
		const values = [];
		const boxX = this.#getXBox(); 
		const boxY = this.#getYBox(); 

		const boxEndingPointX = boxX * 3;
		const boxStartingPointX = boxEndingPointX - 3; 

		const boxEndingPointY = boxY * 3; 
		const boxStartingPointY = boxEndingPointY - 3;

		for(let row = boxStartingPointY; row < boxEndingPointY; row++) {

			for(let column = boxStartingPointX; column < boxEndingPointX; column++) {
				if (column === this.x && row == this.y) continue;
	...
	#getXValues(inputs) {
		return this.#getValues(inputs, this.x, true);
	}

	#getYValues(inputs) {
		return this.#getValues(inputs, this.y, false);
	}

	#generate2DArray(inputs) {
		const arr = [];
		for(let row = 0; row < 9; row++) {
			arr[row] = [];
			for (let col = 0; col < 9; col++) {
				if (row === this.y && col === this.x && !inputs[`${row}${col}`]) {
					arr[row][col] = this.inputVal;
				} else {
					arr[row][col] = inputs[`${row}${col}`]?.inputVal ?? 0;
				}
			}
		}
	...
	isValueValid(inputs) {
		const rowValues = this.#getXValues(inputs);
		const columnValues = this.#getYValues(inputs);
		const boxValues = this.#getBoxValues(inputs);

		const rowsEvalluation = rowValues.includes(this.inputVal);
		const columnEvalluation = columnValues.includes(this.inputVal);
		const boxEvalluation = boxValues.includes(this.inputVal);

		if (isNaN(this.inputVal) || this.inputVal < 1 || this.inputVal > 9 
			|| rowsEvalluation || columnEvalluation || boxEvalluation) 
		{
			return false;
		}
	...
	hasError(inputs) {
		if(this.isValueValid(inputs) && Object.keys(inputs).length > 17) {
			const arrToEvaluate = this.#generate2DArray(inputs);
			return !utilities.isSolvable(arrToEvaluate);
		}
		
		return !this.isValueValid(inputs);
	}
}
...
class utils {
	...
	#existsInRow(inputs, index, value) {

		for(let i = 0; i < 9; i++)
			if (inputs[index][i] === value)
				return true;

		return false;
	}

	#existsInCol(inputs, index, value) {

		for(let i = 0; i < 9; i++)
			if (inputs[i][index] === value)
				return true;

		return false;
	}

	#existsInBox(inputs, row, col, value) {
		const neighbors = [-1, 0, 1]; 
		const centerCellRow = Math.floor(row / 3) * 3 + 1;
		const centerCellCol = Math.floor(col / 3) * 3 + 1;

		for (let y of neighbors)
			for (let x of neighbors)
				if (inputs[centerCellRow + parseInt(y)][centerCellCol + parseInt(x)] === value)
					return true;
	...
	isSolvable(inputs) {
		...
		const {row, col} = this.#nextFreeCell(inputs);

		if (row === -1, col === -1)
			return true;

		for(let i = 1; i < 10; i++) {
			if(!this.#existsInBox(inputs, row, col, i) && !this.#existsInCol(inputs, col, i) && !this.#existsInRow(inputs, row, i)) {
				inputs[row][col] = i;
				
				if (this.isSolvable(inputs)) {
					return true;
				}
